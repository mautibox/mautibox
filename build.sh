...
#	/code/staging 	  	Working copy of the core repo with permissions applied for faster local cloning.
#   /www/data/xxxx.json Current status of the PR in question.
#	/www/data/xxxx.log  Aggregated logs for the PR.
#	/www/home			Home page where you select PRs.
#	/www/data           Status page (views tailed logs via socket)
...
function console {
    ...
    then
        sudo -u webapp bash -c ". /opt/elasticbeanstalk/support/envvars ; /usr/bin/php '$PULL/app/console' $@"
    else
        php "$PULL/app/console" $@
    fi
}
